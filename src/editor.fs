...
let request<'a, 'b> (obj: 'a) endpoint _id =
    ...
    //|> Promise.success(fun r -> (!!r.data<string[]>).[id] |> JS.!!JSON.parse<'b>)
...
let mutable fcsResults: FCS.Results option = None
...
//             promise {
//                 let! o = tooltip { FileName = "test.fsx"; Line = !!position.lineNumber; Column = !!position.column; Filter = "" }
...
//                 let w = model.getWordAtPosition(!!position)
    ...
    new monaco.languages.CompletionItemProvider with
        ...
        member __.provideCompletionItems(model, position, token) =
           async {
                ...
                | Some(parseResults, typeCheckResults, _) ->
                    let! decls = FCS.getCompletionsAtLocation parseResults typeCheckResults 
...
let parseEditor (model: monaco.editor.IModel) =
    ...
    | Some fcsChecker ->
        ...
        fcsResults <- Some(FCS.parseFSharpProject fcsChecker "test.fsx" content)
...
//                         res.range <- monaco.Range (float d.StartLine, float d.StartColumn, float d.EndLine, float d.EndColumn!!)
//                         res.kind <- (!!0)
//                         res)
//                     |> ResizeArray
//             } |> U2.Case2
...
//                 d.range <- monaco.Range(float o.Data.Line, float o.Data.Column, float o.Data.Line, float o.Data.Column!!)
//                 d.uri <- model.uri
//                 return d |> U2.Case1
//             } |> U2.Case2
// }
...
//                         res.range <- monaco.Range (float d.StartLine - 1., float d.StartColumn, float d.EndLine - 1., float d.EndColumn!!)
//                         res)
//                     |> ResizeArray
//             } |> U2.Case2
// }
